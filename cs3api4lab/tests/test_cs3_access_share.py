...
class TestCs3ShareApi(TestCase, LoggingConfigurable):
    ...
    def test_share_file_and_read(self):
        ...
        share_file_path = self.share_mapper.remap_path(marie_list_received['content'][0]['path'])
        marie_file_content = self._read_storage_ext_file(share_file_path)
    ...
    def test_share_container_and_read(self):
        ...
        try:
            share_path = self.share_mapper.remap_path(marie_list_received['content'][0]['path'])
            marie_stat = self.storage_ext.stat(share_path)
    ...
    def test_create_folder_in_share_folder(self):
        ...
        marie_list_received = self.api_ext.list_received()
        share_dir = self.share_mapper.remap_path(marie_list_received['content'][0]['path'])
    ...
    def test_read_write_flag_in_share_folder(self):
        ...
        marie_list_received = self.api_ext.list_received()
        marie_read_dir = list(filter(lambda s: s['path'] == "test_share_dir/test_read_folder", marie_list_received['content']))
        marie_write_dir = list(filter(lambda s: s['path'] == "test_share_dir/test_write_folder", marie_list_received['content']))
